...
- **The Chinese pages** (issue #29): `README.zh.md` and
  ...
  **The translation ships in the commit that changes the English page.**
  ...
  `.codex/agents/docs-translator.toml`.
...
- **Keep the instruction files and agent definitions in sync** (settled
  2026-09-11): any change to `AGENTS.md` or `CLAUDE.md` must update both
  in the same task. Any addition, edit, rename or removal under
...
- **Git housekeeping** (settled 2026-08-28): committing what is in the
  ...
  release (see Releasing) follow `.codex/agents/git-chores.toml`.
...
- **The driving rules are `.codex/agents/zotero-tester.toml`** (the
...
section of `.codex/agents/git-chores.toml` itself. Underneath are two
