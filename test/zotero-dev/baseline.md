...
## 0. Before every case — the baseline
...
- Errors: `Zotero.getErrors()` is a ring — read its **contents**, not its
  ...
  errors from it.
- A verbatim snapshot of every pref the case may touch, restored in
...
- Fixtures: `test/fixtures/fixture-a.pdf` (two pages, 17 segments,
  ...
  Imported as standalone attachments per case
...
- The profile's state is read, not assumed: on 2026-09-05 both WebDAV sync
  switches were on, `Zotero.Debug` was already storing, the
