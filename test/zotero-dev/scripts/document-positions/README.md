...
- **A bare `toggleReadAloudPaused()` reads stale for a tick**
  (`35-read-on-then-pause.js`): `pullBeforePlay` runs its own resume pair
  first, so `paused` still reads `true` right after the call — poll it.
