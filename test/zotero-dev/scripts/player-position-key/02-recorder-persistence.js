return (async () => {
  ...
  const keyButton = settings.doc.getElementById('ztts-key-cyclePlayerLayout'), keyLabel = settings.doc.querySelector('label[data-l10n-id="ztts-key-player-layout"]');
  ...
  if (settingsInitial.layout !== Services.prefs.getStringPref(layoutPref, '') || settingsInitial.shortcut !== 'Shift+P' || settingsInitial.rowLabel !== 'Player layout' || !settingsInitial.help) throw new Error('Settings player-layout row mismatch: ' + JSON.stringify(settingsInitial));
  rows.push({ check: 'Settings label and help', expected: 'Player layout row shows Shift+P and localized help; layout agrees with preference', observed: settingsInitial, status: 'PASS' });
  ...
  rows.push({ check: 'record a new Player layout key', expected: 'Shift+L is written and shown immediately', observed: remap, status: 'PASS' });
  await closeSettings();
  ...
  if (remapPress.keyDown !== 1) throw new Error('remapped Player layout key was not consumed: ' + JSON.stringify(remapPress));
  ...
  if (!conflictMessage?.includes('Player layout') || conflictValue !== speedBefore) throw new Error('conflict did not name Player layout: ' + JSON.stringify({ conflictInput, message: conflictMessage, speedBefore, conflictValue }));
  rows.push({ check: 'shortcut conflict names Player layout', expected: 'conflict message names Player layout and leaves Faster binding unchanged', observed: { input: conflictInput, message: conflictMessage, speedBefore, speedAfter: conflictValue }, status: 'PASS' });
  ...
  if (!cleared) throw new Error('Clear did not remove Player layout binding');
  await closeSettings();
  ...
  if (oldPress.keyDown === 1 || newPress.keyDown === 1 || Services.prefs.getStringPref(layoutPref, '') !== 'A') throw new Error('cleared Player layout binding still acted: ' + JSON.stringify({ oldPress, newPress, layout: Services.prefs.getStringPref(layoutPref, '') }));
  rows.push({ check: 'Clear disables Player layout shortcut', expected: 'Shift+P and remapped Shift+L both fall through after Clear', observed: { oldPress, newPress, layout: Services.prefs.getStringPref(layoutPref, '') }, status: 'PASS' });
