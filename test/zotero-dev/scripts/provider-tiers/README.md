...
| 2026-09-24 | 1.14.4-beta6 (issue #134 regression) | plugin-player kit's report | 1 PASS (`provider entries in the player` ok; `labels` has all 12 keys), 2 PASS (`resolveShadow`/`tierMemoryHook` true, `tiers:["fish"]`, `retagged:{fish:339}`, `patches().providerTiers` 5/5), 3 PASS (`pluginPlayer().state.providers` = `[{fish,"Fish Audio"}]`, no `local`) | Ad hoc reads on `fixture-a.pdf` (only Fish enabled this profile, per "never switch one on"; no `createElementWrapped`/`options`/`loginRowReplaced` keys, confirming ADR 0007) reusing this kit's own `00`/`02`/`03` shape, not the numbered scripts themselves — items 4-8 out of scope |
