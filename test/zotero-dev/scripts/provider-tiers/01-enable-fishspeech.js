// Item 2/3/7 prep for the #111/#112 labels amendment: enable Fish Speech
// (issue #112 split it out of the old combined Fish section) through the
// settings pane's own Enable button (ui/provider-rows.ts onToggle), which
// runs the connection check first and only then writes fishspeech.enabled.
// Configured in this profile (baseURL set, read for the record only, never
// printed) -- authorized per the tester rulebook's "configured providers"
// rule. Falls back to a raw pref write, noted in the output, if the pane
// route fails. Closes the settings window it opened. Idempotent: does
// nothing but report if fishspeech.enabled is already true.
// params: none. state: reads nothing, writes nothing to state.
(async () => {
  const out = { step: 'enable-fishspeech' };
  const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
  const PREF = 'extensions.zotero.zotero-tts.fishspeech.enabled';
  let win = null;
  try {
    out.enabledBefore = Services.prefs.getBoolPref(PREF, false);
    out.baseURLConfigured = !!Zotero.Prefs.get('zotero-tts.fishspeech.baseURL', true);
    ...
    const toggle = doc.getElementById('ztts-enable-fishspeech');
    const result = doc.getElementById('ztts-test-result-fishspeech');
    if (!toggle) throw new Error('no #ztts-enable-fishspeech in the pane');
