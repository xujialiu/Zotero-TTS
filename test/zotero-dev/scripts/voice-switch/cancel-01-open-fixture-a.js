return (async () => {
  ...
  const title = 'Zotero-TTS issue 95 follow-up fixture A ' + Date.now();
  ...
  if (!item?.id) throw new Error('fixture A import returned no item');
  fixture.itemID = item.id;
  fixture.key = item.key;
  fixture.title = title;
  const opened = await root.openFixture(fixture, false);
  return JSON.stringify({ fixture: 'a', itemID: item.id, key: item.key, title, opened, state: root.read(fixture) }, null, 1);
})()
