(async () => {
  ...
  const target = (() => { const voices = manager._allVoices || []; for (let i = 0; i < voices.length; i++) { try { const label = String(voices[i]?.label ?? voices[i]?.name ?? ''); if (String(voices[i]?.tier ?? '') === 'system' && /Ava \(Enhanced\)/.test(label)) return { id: String(voices[i].id), label }; } catch (_) {} } return null; })();
  if (!target) throw new Error('listed System Ava target is missing');
  ...
  if (!option) throw new Error('System Ava row is missing from player');
  option.click();
