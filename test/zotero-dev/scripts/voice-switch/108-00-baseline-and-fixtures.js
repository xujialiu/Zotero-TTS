return (async () => {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    readers.push({
      itemID: reader.itemID, tabID: reader.tabID, title,
  ...
  const fixturesDir = String(p.fixturesDir || '').replace(/\//g, '\\');
  const specs = [
  ...
  for (const spec of specs) {
    const title = `Zotero-TTS issue 108 ${spec.kind} ${Date.now()}`;
  ...
  return JSON.stringify({
    status: 'PASS', zotero: Zotero.version, fixtures: fixtures.map(({ file, ...rest }) => rest),
    ownerReaders: readers, prefs: summary, volume: Zotero.Prefs.get('zotero-tts.readAloud.volume'),
