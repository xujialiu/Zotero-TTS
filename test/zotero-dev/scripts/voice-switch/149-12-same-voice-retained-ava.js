(async () => {
  ...
  const target = { id: manager.selectedVoiceID, label: 'Ava (Enhanced)' };
  if (!target.id || !target.id.includes('Ava')) throw new Error('Ava is not the retained selection');
  ...
  const option = [...(menu?.querySelectorAll('button.option') || [])].find(e => /Ava \(Enhanced\)/.test((e.textContent || '').trim()));
  if (!option) throw new Error('retained Ava row is missing from player');
  option.click();
  ...
  return JSON.stringify({ status: recovered ? 'PASS' : 'FAIL', target, beforePosition, before, after, positionUnchanged: beforePosition === after.position, noRequestsBeforePlay: after.requests === 0 && after.calls === 0, retainedID: before.selected === target.id, actualPlayerControl: true }, null, 1);
})();
