// Baseline for issue #111 (zotero-tiers): Zotero/reader state before any
...
// muted to 0. Never writes webdav.syncSettings (recorded only, mirrors the
// provider-tiers kit's own rule even though this case does not list it).
...
(async () => {
  ...
  try {
    ...
    out.applied = { volume: readPref(baseline.volume.key).value };
  } catch (e) {
    out.error = String(e);
