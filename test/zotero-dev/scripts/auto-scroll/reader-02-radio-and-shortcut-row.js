return (async()=>{
  ...
  while(Date.now()<end&&!win.document.getElementById('ztts-provider-openai-official')) await new Promise(r=>setTimeout(r,100));
  ...
  return JSON.stringify({loaded:!!d.getElementById('ztts-provider-openai-official'),before,rows,afterSentence,afterOutside,uiValue:g?.value??null,shortcut:{exists:!!shortcut,text:shortcut?.label??shortcut?.textContent??null,disabled:!!shortcut?.disabled},clear:{exists:!!clear,disabled:!!clear?.disabled}});
})()
