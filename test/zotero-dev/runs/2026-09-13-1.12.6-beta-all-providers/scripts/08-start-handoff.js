return (async () => {
  ...
  try { rw.document && rw.document.notifyUserGestureActivation && rw.document.notifyUserGestureActivation(); } catch (e) { out.errors.push('activation: ' + String(e)); }
  try { const context = manager._controller && manager._controller._audioContext; if (context && context.state === 'suspended') await Promise.race([context.resume(), new Promise(resolve => setTimeout(resolve, 1000))]); } catch (e) { out.errors.push('resume: ' + String(e)); }
  let startKeys = null;
