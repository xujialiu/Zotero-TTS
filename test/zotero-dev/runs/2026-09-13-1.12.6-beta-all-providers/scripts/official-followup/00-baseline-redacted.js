return (async () => {
  ...
  const userReader = list.length ? list[0] : null;
  const proto = userReader && Object.getPrototypeOf(userReader);
  const descriptor = proto && Object.getOwnPropertyDescriptor(proto, '_getReadAloudRemoteInterface');
  const baseline = {
    prefs,
    readers,
    main: { selectedID: main && main.Zotero_Tabs && main.Zotero_Tabs.selectedID || null, selectedIndex: main && main.Zotero_Tabs && main.Zotero_Tabs.selectedIndex || null },
    debugStoring: !!Zotero.Debug.storing,
    position,
    nativeStub: descriptor && typeof descriptor.value === 'function' ? { proto, descriptor } : null
  };
  Zotero.__zttsOfficialFollowup = { baseline, fixture: null, spec: null, run: null, results: [], crossTier: null };
  const redacted = {};
  ...
  return JSON.stringify({ stored: true, zoteroVersion: Zotero.version, platform: Services.appinfo.OS, readers, main: baseline.main, nativeStub: !!baseline.nativeStub, debugStoring: baseline.debugStoring, prefs: redacted, position: { rows: position && position.database && position.database.rows || null, queued: position && position.store && position.store.queued || null, lastError: position && position.store && position.store.lastError || null } }, null, 1);
})()
