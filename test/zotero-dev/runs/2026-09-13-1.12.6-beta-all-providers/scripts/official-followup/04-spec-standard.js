return (async () => {
  ...
  const out = { status: 'FAIL', errors: [], tier: 'standard' };
  ...
  try { manager.selectTier('standard'); } catch (e) { out.errors.push('select-tier: ' + String(e)); }
  await sleep(150);
  ...
  if (rows.length < 2) { out.status = 'NOT TESTABLE'; out.errors.push('standard current-language pool has fewer than two voices'); out.pool = rows; return JSON.stringify(out, null, 1); }
  ...
  await sleep(100);
  state.spec = { tier: 'standard', direction: 1, source: sourceVoice, target: targetVoice, pool: rows.slice(0, 12), segmentIndex, segmentText: String(segments[segmentIndex] && segments[segmentIndex].text || '') };
  out.status = !out.errors.length && manager._selectedTier === 'standard' && manager.selectedVoiceID === sourceVoice.id ? 'PASS' : 'FAIL';
