return (async () => {
  ...
  const errorEntries = Zotero.getErrors ? (Zotero.getErrors() || []) : [], pluginErrorEntries = [], deadErrorEntries = [];
  for (let i = 0; i < errorEntries.length; i++) { const entry = typeof errorEntries[i] === 'string' ? errorEntries[i] : JSON.stringify(errorEntries[i]); if (/\[zotero-tts\]|zotero-tts\.js/i.test(entry)) pluginErrorEntries.push(entry); if (/can't access dead object/i.test(entry)) deadErrorEntries.push(entry); }
  ...
  out.status = out.errors.length || !out.final.selectedTabRestored || !out.final.nativeStubRestored ? 'FAIL' : 'PASS';
