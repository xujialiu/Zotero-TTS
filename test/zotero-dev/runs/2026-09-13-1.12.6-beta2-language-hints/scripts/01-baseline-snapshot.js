(async () => {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    try { position = manager && manager._controller ? manager._controller._position : null; } catch (e) {}
    try { lastError = manager && manager._controller ? manager._controller._lastError : null; } catch (e) {}
    readers.push({
      itemID: r.itemID,
      title,
