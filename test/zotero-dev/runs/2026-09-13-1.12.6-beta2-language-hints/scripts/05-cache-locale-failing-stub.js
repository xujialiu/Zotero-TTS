(async () => {
  ...
  const calls = [];
  let delayed = false;
  const audio = Uint8Array.from([255, 251, 144, 1, 0, 0, 0, 0]);
  ...
  function responseFor(bodyText) {
    ...
    // Exact failing harness: the following two characters are literal \n
    // because this script was passed through an outer template string.
    const payload = 'data: ' + JSON.stringify({ audio_base64: base64, chunk_seq: 0, alignment: { segments: words } }) + '\\n\\n';
  ...
  try {
    ...
    await Promise.all([iface.getAudio(concurrentText, voice), iface.getAudio(concurrentText, voice), iface.getAudio(concurrentText, { ...voice, locale: 'en-GB' })]);
