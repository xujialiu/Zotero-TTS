return (async()=>{
  ...
  const w=rr._window;
  const before=JSON.parse(Zotero.ZoteroTTS.diagnostics.autoScroll())[0];
  let wheelError=null;
  try{w.windowUtils.sendWheelEvent(800,600,0,120,0,0,0,0,0,0);}catch(e){wheelError=String(e);}
  await new Promise(r=>setTimeout(r,400));
  const disengaged=JSON.parse(Zotero.ZoteroTTS.diagnostics.autoScroll())[0];
  let playError=null;
  try{m.play();}catch(e){playError=String(e);}
  await new Promise(r=>setTimeout(r,450));
  const resumed=JSON.parse(Zotero.ZoteroTTS.diagnostics.autoScroll())[0];
