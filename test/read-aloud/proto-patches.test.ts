...
describe('createProtoPatches', () => {
  ...
  // BrowserReadAloudVoice.label is a prototype accessor (reader.js:39629),
  // which shadow()'s plain assignment cannot replace and restoreAll()'s
  // cannot put back (issue #9)
  it('shadows a getter and puts the original accessor back', () => {
    const { patches, error } = make();
    const proto = { get label() { return 'Microsoft David'; } } as any;
    const original = Object.getOwnPropertyDescriptor(proto, 'label')!.get;
    const voice = Object.create(proto);

    patches.shadowGetter(proto, 'label', (o) =>
      function (this: unknown) {
        return 'Local-' + Reflect.apply(o, this, []);
      },
    );
    expect(voice.label).toBe('Local-Microsoft David');

    patches.restoreAll();
    expect(voice.label).toBe('Microsoft David');
    expect(Object.getOwnPropertyDescriptor(proto, 'label')!.get).toBe(original);
    expect(error).not.toHaveBeenCalled();
  });

  it('leaves the getter alone when the name is not an accessor of that prototype', () => {
    const { patches, error } = make();
    const proto = liveProto();
    const make_ = vi.fn((o: any) => o);

    patches.shadowGetter(proto, 'm', make_);
    patches.shadowGetter(proto, 'absent', make_);

    expect(make_).not.toHaveBeenCalled();
    expect(patches.counts()).toEqual({ total: 0, live: 0 });
    expect(error).not.toHaveBeenCalled();
  });

  it('holds a getter and a method of the same prototype as separate entries', () => {
    const { patches } = make();
    const proto = { m: function original() {}, get label() { return 'x'; } } as any;

    patches.shadow(proto, 'm', (o) => o);
    patches.shadowGetter(proto, 'label', (o) => o);

    expect(patches.has(proto, 'm')).toBe(true);
    expect(patches.has(proto, 'label')).toBe(true);
    expect(patches.counts()).toEqual({ total: 2, live: 2 });
  });

  it("exports the getter into the reader's compartment when exportFunction is given", () => {
    const exported = function exportedGetter() {
      return 'exported';
    };
    const exportFunction = vi.fn(() => exported);
    const { patches } = make({ exportFunction });
    const proto = { get label() { return 'x'; } } as any;

    patches.shadowGetter(proto, 'label', (o) => o);

    expect(exportFunction).toHaveBeenCalledTimes(1);
    expect(proto.label).toBe('exported');
  });

  it("leaves a closed tab's accessor alone, and says nothing about it", () => {
    const { patches, error, dead } = make();
    const proto = { get label() { return 'x'; } } as any;
    patches.shadowGetter(proto, 'label', () => function () {
      return 'Local-x';
    });
    ...
    expect(proto.label).toBe('Local-x');
    expect(error).not.toHaveBeenCalled();
    expect(patches.counts()).toEqual({ total: 0, live: 0 });
  });
