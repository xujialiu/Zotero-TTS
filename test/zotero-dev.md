...
1. **The section.** Between Speechify and Kokoro-FastAPI, one groupbox
   ...
   `h3.ztts-subheading`, `Cloud (api.fish.audio)` and `Fish Speech server
   (your own)`. The cloud block `ztts-provider-fish`: a password input on
...
3. **Pasted voices.** `fish.voices` =
   ...
   `local`: `Fish-jjk narrator` / `fish::en/179b…` / language `en`,
   `Fish-ffffffffffffffffffffffffffffffff (not found)` /
   `fish::mul/ffff…` / `mul`, and `Fish-Default` / `fish::mul/default` /
   `mul`.
...
7. **Chinese: one timestamp per character.**
   ...
   with `Fish-Default`: `fish: 6 word timestamps for 6 chars`, `15 for
...
10. **Pending a server** (nothing to run until a fish-speech
    `tools/api_server.py` is up): `/v1/references/list` publishing every
    reference folder as a voice under Multiple languages with no title
    and no language; Enable succeeding and locking Address and Extra
    headers; a sentence read through `POST /v1/tts` highlighted **by
    sentence, never by word** (`capabilities.wordTimestamps: false`); a
    server started with `--api-key` answering 401 `Invalid token` until
    `Authorization: Bearer …` is put in Extra headers.
11. **Leave the state as the owner had it**: the key and `fish.enabled`
    as before the pass, `fish.voices` without the bogus id, the debug
    store back off, the fixtures erased. A pref write here fires the
    owner's settings auto-upload when it is on: the key travels with it.
...
The plugin shadows `navigateToPosition` on the PDF view's prototype per
tab and answers the follow's own calls (`ifNeeded`, `inline: 'nearest'`,
a negative `visibilityMargin` — reader.js:76472 in 10.0.2-beta.9): it
measures the whole sentence, both pages, keeps Zotero's trigger, adds
"any part outside the viewport less the margin", and scrolls
`#viewerContainer` itself (`src/read-aloud/sentence-in-view.ts`). The
...
1. **Attached.** With the player open: the debug line `sentence in view
   attached to a PDF view` for the tab, `diagnostics.sentenceInView()` →
   `kind: 'pdf', patched: true` for every open PDF reader, and
...
4. **Shift+Enter brings the tail in.** Scroll by script so sentence 82's
   head top sits in the middle half of the viewport and its tail is off
   screen — that unlocks; pause first and wait ~1.2 s, since a scroll
   made while a push is in flight is absorbed and the follow pulls the
   view back — no plugin line while unlocked; then a trusted
   Shift+Enter (§4.4's TIP): `keydown` = 1, `return to spoken: pdf view,
   state kept`, then `sentence in view: cut on page 2: … -> 2768`,
   `cut: false`, `locked: true`.
...
7. **The user's scroll still unlocks.** Locked and settled, scroll the
   container by script by 300 px: `_readAloudPositionLocked` false at
   the 100 ms sample, and `last.at` frozen even across a
   `skipAhead('sentence')` push — the shadow is not called at all.
   `skipAhead('sentence')` + `_lockPositionToReadAloud()` re-locks and
   `last.at` moves again.
