...
2. **The pane renders.** Open per the rulebook; screenshot every group:
   the five provider sections (OpenAI, Azure, Cloudflare Workers AI, the
   Local engine named after its engine — `Kokoro-FastAPI`, System
   ...
   *visible* note: the hidden platform notes collapse to a zero rect and
   ...
   `l10n().pane.elements` is 114 (the file's 115 `data-l10n-id`
   occurrences less the one in its header comment: 100 at 1.11.0, plus
   the volume setting's nine, the star line's one and, at 1.11.3, the
   Cloudflare section's four). The message lines
...
3. **Locked sections, masked fields.** Every enabled provider: inputs
   `disabled`, button `Disable`; the two API keys, the Cloudflare token,
...
15. **Errors at the end**, by content.
...
20. **Teardown**: popup closed, tab closed, the item erased, `rows` back.
