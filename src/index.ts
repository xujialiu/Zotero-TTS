...
/**
 * Every reader's interface goes through this one wrapper (read-aloud/
 * window-interface.ts): the window's own promise and a clone of the
 * result, as Zotero's native interface does — and a result that lands
 * after its window is gone is dropped and counted (issue #116), the
 * count the `patches` diagnostic reports as `lateResults`.
 */
const windowWrapper = createWindowWrapper({
  cloneInto: (value, target) => Components.utils.cloneInto(value, target, { cloneFunctions: false }),
...
function startReadAloudMemory(): void {
  ...
  readAloudMemory = createReadAloudMemorySync({
    ...
    // The local engine's name: the tier its voices are filed under (issue #110)
    localEngine: () => providerNaming(loadSettings(prefs)).localEngine,
...
function startSkippedLines(): void {
  ...
  skippedLines = createSkippedLines({
    ...
    // The promise Zotero awaits is its own window's, the way read-aloud/window-interface.ts answers it
...
// ---- One entry per provider in the player's first dropdown -----------------
//
...
const diagnostics = {
   ...
   * any pref of the old section still holding a value the user set —
   * none after a split — and the old prefs whose default is still
   * registered from the old prefs.js (`staleDefaults`), which the split
   * ignores. Synchronous.
   */
  openaiSplit(): string {
    const settings = loadSettings(prefs);
    const masked = (id: (typeof SPLIT_TARGETS)[number]) =>
      Object.fromEntries(
        Object.entries(settings[id]).map(([field, value]) => [field, field === 'apiKey' || field === 'headers' || field === 'baseURL' ? String(value).length : value]),
      );
    return JSON.stringify(
      {
        ...
        legacyPrefs: LEGACY_OPENAI_FIELDS.filter((field) => legacyPrefSet(prefs, field)),
        staleDefaults: LEGACY_OPENAI_FIELDS.filter((field) => !legacyPrefSet(prefs, field) && prefs.get(PREF_PREFIX + LEGACY_OPENAI_PREFIX + field) !== undefined),
      },
      null,
      1,
    );
    ...
    JSON.stringify(
      {
        ...
        // Results that landed after their reader window was gone, dropped instead of thrown (issue #116)
        lateResults: safe(() => windowWrapper.report()) ?? null,
      },
      null,
      1,
    ),
  ...
  defaultVoice: async () => {
    try {
      ...
      const { voices, problems, columns } = await listBrowserVoices({
        ...
        // The first column as the pane shows it: every enabled provider and Zotero's two (issue #110)
        tierColumns: () => providerTierColumns(loadSettings(prefs)),
      });
      const rows = defaultVoiceRows(voices, memory.voice);
      const home = rows[0] ?? null;
      const tiers = groupVoicesByTier(voices, appLanguageName, columns);
      return JSON.stringify(
        {
          memory,
          globalSpeed,
  ...
  languageColumn: async () => {
    try {
      ...
      const tiers = groupVoicesByTier(browserVoices(catalog, zotero), appLanguageName, providerTierColumns(loadSettings(prefs)));
