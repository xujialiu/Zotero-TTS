/**
 * Where a resume after a long pause starts: Read Aloud's own word-onset
 * search, copied unchanged from Zotero 10.0.3 (ADR 0006, parity checklist
 * item 11 of issue #133). A pause of 5 s or more resumes one word back, of
 * 20 s or more two, for voices whose locale starts with `en`.
 *
 * Source: zotero/reader, `src/common/read-aloud/remote/lib/word-onset.ts`,
 * as bundled in Zotero 10.0.3's `resource/reader/reader.js` (39821-39894).
 ...
 * annotations are added; test/core/engine/word-onset.test.ts proves the
 * answers identical to the bundle's own.
 */

import type { PcmBuffer } from './time-stretch';

// ---- Copied from Zotero 10.0.3, reader.js 39822-39894 ----------------------
 ...
 */
export function findWordOnset(buffer: PcmBuffer, position: number, wordBoundaries = 1): number {
