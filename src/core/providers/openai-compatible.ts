...
/**
 * The client the OpenAI, Xiaomi MiMo and OpenAI Compatible sections share
 * (issue #113): OpenAI's speech API as every server that speaks it has it —
 * `/v1/audio/speech` for the bytes, `/v1/models` as the cheapest
 * authenticated probe, `/v1/audio/voices` where a server publishes its
 * voices — plus the chat completions route that carries audio as base64,
 * which is all Xiaomi MiMo's TTS has (issue #50). What used to be read off
 * the hostname is said by the caller: whether a key is required, the route,
 * the voices the server documents, the name its errors carry.
 */
...
export type OpenAICompatibleConfig = {
  /** The provider's id: the voice ids' prefix and the player's tier key. */
  id: ProviderId;
...
export function createOpenAICompatibleProvider(cfg: OpenAICompatibleConfig, deps: { fetch: typeof fetch }): TTSProvider {
  ...
  async function speak(text: string, voice: string, signal?: AbortSignal): Promise<{ audio: Blob; note?: string }> {
    const chat = cfg.route === 'chat';
    const what = chat ? 'chat/completions audio' : `${cfg.label} speech`;
    const body = chat
  ...
  const provider: TTSProvider = {
     ...
     * caller documents for it (OpenAI's own list, MiMo's). A server that
     * cannot be reached at all is an error, not a reason to show the
     * documented names for it.
     */
