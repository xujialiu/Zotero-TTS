<!-- translated-from: README.md sha256:f87b53d2c383 -->
...
- *自动滚动*——适用于 PDF 和 EPUB，可在*高亮*设置中选择*每句居中*或*超出视图时滚动*（默认）。按下 `Shift+A` 可切换模式；当前模式会短暂显示。第一种在句子开始时将整句居中；第二种让完全可见的句子保持原位，只有在句子有部分超出视图时才将其居中。逐词高亮不会让一个能完整显示的句子逐词移动。比视图高的句子从句首开始，然后在有真实的单词时间戳时跟随当前单词；没有时，只能可靠地定位句首。翻页 EPUB 保留各自的页面。文档边界可能限制居中。
