<!-- translated-from: README.md sha256:0ab7869af30a -->
...
- *朗读时去掉包围文字的尖括号*——默认对所有语音开启。`<Hello>.` 会按 `Hello.` 朗读；括号外的标点保留。多个括号组也适用：`<Log in> <Register>` 会按 `Log in Register` 朗读。如果括号组外还有普通文字，则整句保留原文。文档不会改变。修改后，停止并重新打开朗读即可生效。
