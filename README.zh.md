<!-- translated-from: README.md sha256:c9a6765726b9 -->
...
<details>
<summary><b>统一语音与速度、停顿、预取、缓存、整句都在屏幕内</b></summary>
...
- *整句都在屏幕内*——在 PDF 里，正在朗读的句子如果会超出窗口底部、延伸到下一页或下一栏，视图就会跟着滚动，让整句都在屏幕内；比窗口还高的句子，则跟随正在朗读的词滚动。始终开启。手动滚动视图后，自动跟随仍然会停住，直到按 `Shift+Enter`，或者用播放器的跳转按钮。
...
- *在电脑之间同步设置*是双向的：这里的改动几秒后就到其他电脑，那边的改动也会到这里，不用手动恢复。默认关闭。
- *各台电脑自己保留的：*本机或局域网地址的语音服务器、*系统语音*的开关，以及这个 WebDAV 连接。
- *两台电脑改动同一项设置：*以较晚的改动为准。
- 不管是恢复到的那台电脑上，还是同步带来改动的那台电脑上，用不了的服务商都会保持关闭，并说明原因。
