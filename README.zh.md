<!-- translated-from: README.md sha256:918dd010d564 -->
...
<p align="center">如果你喜欢 Zotero-TTS，欢迎给它点个 ⭐——让更多人发现它。</p>
...
- ⌨️ **键盘快捷键**：加速、减速、复位；音量加、音量减；按句或按段跳转；从选中的文字开始读；把视图拉回正在朗读的地方；打开播放器的选项面板。全部可以改键。[→ 键盘快捷键](#键盘快捷键)
...
Windows 和 macOS 都自带一些语音，朗读也早就把它们列在**本地**里。Zotero 是直接从浏览器引擎拿的，那条路既不交出音频也不交出词级时间戳——所以这些语音恰恰是插件唯一碰不到的一类：没有语音浏览器、没有试听、没有 ♡、没有缓存、没有逐词高亮。

*系统语音*那一节里的**启用**把它们接管过来。插件自己去跟系统的语音引擎对话——Windows 上通过一个小的辅助进程，macOS 上通过 `say`——再把结果当作普通的插件语音公布出来，名字形如 `System-Microsoft David`、`System-Samantha`。Windows 上它们带词级时间戳，这比 Zotero 自己那条路能给的多；macOS 上按句高亮，和这些语音在 Zotero 自己那条路上一样。

不管这个开关开不开，Zotero 自己那几份同名语音都不会出现在播放器里：插件把它们藏了，因为 Zotero 是通过浏览器引擎去用它们的，插件为一个语音做的任何事都跟不过去。如果你原先选的正是那几份里的某一个，启用后它会被改指到插件的对应语音，于是照读不误。在 Linux 上，插件没有辅助进程，所以播放器提供的是 Zotero 自己的标准、高级语音，加上你配好的那些服务商。
