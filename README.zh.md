<!-- translated-from: README.md sha256:5aefc2e2b54f -->
...
- 🧵 **分段不分句**——Zotero 有时会在一句话中间把段落切断，前后两半会被当作一句朗读、一句高亮。[→ 朗读](#朗读)
- 🎧 **设置里的语音浏览器**：按语音模式和语言列出每一个语音，一个播放按钮试听，一颗心收藏，还有一个开关让播放器只提供收藏的语音。[→ 语音浏览器](#语音浏览器)
- ✨ **逐词高亮和逐句高亮同时进行**，颜色和不透明度都由你定——Zotero 自己的语音也一样。[→ 高亮](#高亮)
- ⌨️ **键盘快捷键**：调速、调音量、按句或按段跳转、从选中的文字开始读、打开播放器的选项面板、自动滚动模式、开关单词高亮、一次停止所有标签页的朗读。全部可以改键。[→ 键盘快捷键](#键盘快捷键)
...
- **上一个声音/下一个声音**——`Shift+,` / `Shift+.` 按播放器当前声音列表循环切换，到列表任一端会绕回。播放时，旧声音会继续朗读，等新声音准备好后，在单词结束处交接；必要时改在句子之间交接。暂停时只更换声音，不试听样本。一条简短的提示会显示声音名称。
- **正在准备声音时**，再次按键会更改目标；暂停、跳转、调速或手动选择声音都会取消待处理的切换。准备声音可能消耗服务额度，即使你随后选择了其他声音。
...
<details>
<summary><b>打开时展开播放器、统一语音与速度、停顿、预取、缓存、整句都在屏幕内、页首一行、切成两句的话</b></summary>
...
- *朗读时去掉包围文字的括号*——默认对所有语音开启，默认括号对为 `<> []`。`<Hello> [World].` 会按 `Hello World.` 朗读，保留外面的标点和间隔。取消勾选后可以编辑列表，多个括号对之间用空格分隔；重新勾选时会检查并启用。输入无效时，可选择“使用默认值”，或选择“取消”继续编辑。如果括号组外还有普通文字，则保留原文。文档不会改变。修改后，停止并重新打开朗读即可生效。
- *预取后面的 N 句*——当前句播放时提前合成后面的句子，播放就不必等服务器。它要用到下面的缓存，所以会让缓存保持开启。
- *缓存已合成的音频*——往回跳或者重开文档时不再产生新的请求。存在内存里（64 MB）；Zotero 重启就清空。
- *自动滚动*——适用于 PDF 和 EPUB，可在*高亮*设置中选择*每句居中*（默认）或*超出视图时滚动*。按下 `Shift+A` 可切换模式；当前模式会短暂显示。第一种在句子开始时将整句居中；第二种让完全可见的句子保持原位，只有在句子有部分超出视图时才将其居中。逐词高亮不会让一个能完整显示的句子逐词移动。比视图高的句子从句首开始，然后在有真实的单词时间戳时跟随当前单词；没有时，只能可靠地定位句首。翻页 EPUB 保留各自的页面。文档边界可能限制居中。
- *边听边浏览*——默认情况下，手动滚动或翻页只有在整句高亮离开视图后才会暂停跟随。只要还有一部分可见，就会继续跟随；你移动页面时，自动滚动会暂时等待。当前句子有任意一部分重新可见时，移动页面结束后会自动恢复跟随；朗读进入已经在视图内的句子时也同样如此。关闭*句子可见时保持自动滚动*后，滚动或翻页会立即暂停跟随，只会在你主动请求时恢复。*回到朗读位置*（默认为 `Shift+Enter`）会返回并恢复所选模式；播放器的跳转按钮也会恢复跟随。更改任一设置不会中断音频，也不会在当前句子位于视图外时强行把页面移回去。自动滚动、缩放和窗口变化不会自行关闭跟随。这两项设置会随其他设置保存、备份和同步。
- *Zotero 漏读页首一行时，补读这一行*——一句话跨到下一页时，那一页的第一行可能被 Zotero 漏掉：朗读直接跳过去，接起来听着仍像一句话。开启时这一行照常朗读、照常高亮。若某页的页眉被读了出来，就关掉它。对之后打开的文档生效。
- *Zotero 把一句话切成两句时，合成一句朗读*——Zotero 有时会在一句话中间把段落切断，把两半当作两句朗读，中间还停顿一下。开启时两半作为一句朗读、一句高亮。若两个段落被连成一句读了出来，就关掉它。对之后打开的文档生效。
