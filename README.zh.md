<!-- translated-from: README.md sha256:95aa74cddda9 -->
...
- **使用插件播放器**默认开启。点击阅读器中的红色耳机图标，打开播放器并开始朗读；再次点击停止并关闭。关闭此设置后使用 Zotero 原来的播放器。
- **底部栏、顶部栏、悬浮面板**可在设置中选择，也可从播放器的布局按钮选择。栏只在文档区域占用空间；悬浮面板可以拖动。没有保存过布局时，默认使用顶部栏；已有选择会保留。
...
- **播放器位置 — `Shift+P`**插件播放器打开时，无论正在播放还是暂停，都可按顶部栏 → 底部栏 → 悬浮面板的顺序循环切换。所有播放器共用此位置，重启后仍会保留。长按只切换一次；在文本框中输入时不改变位置。
...
- *打开时展开播放器*——每次打开悬浮面板时显示语音服务、语言和声音三行；使用 Zotero 原播放器时展开其选项面板。默认关闭。点击选项或按 Shift+O 可在本次打开期间收起或展开。设置更改在下次打开播放器时生效。
