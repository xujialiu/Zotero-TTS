<!-- translated-from: README.md sha256:f690af34fc83 -->
...
- *边听边浏览*——默认情况下，手动滚动或翻页只有在整句高亮离开视图后才会暂停跟随。只要还有一部分可见，就会继续跟随；你移动页面时，自动滚动会暂时等待。关闭*句子可见时保持自动滚动*后，滚动或翻页会立即暂停跟随。*回到朗读位置*（默认为 `Shift+Enter`）会返回并恢复所选模式；播放器的跳转按钮也会恢复跟随。更改任一设置不会中断音频，也不会在手动浏览后自动恢复跟随。自动滚动、缩放和窗口变化不会关闭跟随；回到隐藏的文档时，只有在你没有主动浏览离开时才会追上当前朗读位置。这两项设置会随其他设置保存、备份和同步。
