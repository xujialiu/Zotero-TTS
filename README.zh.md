<!-- translated-from: README.md sha256:f33ac663f00f -->
...
- *边听边浏览*——手动滚动或翻页会暂停跟随。*回到朗读位置*（默认为 `Shift+Enter`）会返回并恢复所选模式；播放器的跳转按钮也会恢复跟随。更改模式不会中断音频，也不会在手动浏览后自动恢复跟随。自动滚动、缩放和窗口变化不会关闭跟随；回到隐藏的文档时，只有在你没有主动浏览离开时才会追上当前朗读位置。该模式会随其他设置保存、备份和同步。
