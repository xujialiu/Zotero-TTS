<!-- translated-from: README.md sha256:e2b4e40d5d10 -->
...
- **预计剩余朗读时间**显示在所有布局中，默认在朗读设置中启用。它会显示全文和当前目录部分；朗读选中文本时则只显示所选内容。估算随语速调整，并在朗读中逐步修正；手动暂停和网络等待期间不倒数。没有可靠边界的部分不会显示。
