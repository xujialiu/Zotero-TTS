...
- **Sampling polls; it does not wrap.** `_state.readAloudState.savedPosition`
  ...
  before, and a bookmark one sentence stale is not worth that. The tick is a
  recursive `setTimeout` because its interval changes with activity, not
  because `setInterval` is missing: the sandbox has it (see the whitelist
  above; re-verified 2026-08-30 against the live sandbox global). This entry
