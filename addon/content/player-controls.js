...
let state = { provider: '', locale: '', voice: '', speed: 1, volume: 100, automatic: true, playing: false, active: false,
...
function render() {
  ...
  $('#player-root').innerHTML = `<section class="player ${variant === 'B' ? 'layout-B' : 'layout-A'} ${variant === 'top' ? 'layout-top' : ''}" aria-label="Zotero-TTS">
    <div class="identity"><span>Zotero-TTS</span><span class="grip" aria-hidden="true">⠿</span></div>
    <button class="play" type="button"></button>
    <div class="voice-group">${field('provider')}${field('locale')}${field('voice')}</div>
    <div class="controls"><button class="adjust" data-adjust="speed" aria-haspopup="dialog"></button><button class="adjust" data-adjust="volume" aria-haspopup="dialog"></button><button class="adjust mode"></button><button class="status-button" hidden>!</button><button class="adjust layout-menu" aria-haspopup="dialog" aria-expanded="false"><svg width="18" height="18" viewBox="0 0 20 20" fill="none" aria-hidden="true"><rect x="2.5" y="3.5" width="15" height="13" rx="1" stroke="currentColor" stroke-width="1.25"/><path d="M3 7h14M3 13h14" stroke="currentColor" stroke-width="1.25"/></svg></button></div>
  </section>`;
  ...
  if (variant === 'B') makeDraggable($('.identity'));
  updateControls();
}
...
function place() {
  ...
  if (variant === 'A') {
    ...
    popover.style.top = Math.max(10, rect.top - height - 8 > 50 ? rect.top - height - 8 : Math.min(rect.bottom + 8, innerHeight - height - 10)) + 'px';
  }
...
function openPicker(button, key) {
  ...
  const list = document.createElement('div'); popover.append(list);
  function update() {
  ...
  refreshPopover = update; if (search) search.oninput = update; update();
  (search || list.querySelector('button'))?.focus({ preventScroll: true });
}
function openRange(button, key) {
  if (!createPopover(button, text(key))) return;
  const speed = key === 'speed';
  popover.innerHTML = `<div class="range-title"><span>${escapeHTML(text(key))}</span><strong></strong></div><input type="range" min="${speed ? .5 : 0}" max="${speed ? 3 : 100}" step="${speed ? .1 : 1}"><div class="presets"></div>`;
  ...
  for (const value of speed ? [.75, 1, 1.2, 1.5, 2] : [0, 25, 50, 80, 100]) {
...
window.zttsUpdate = json => { const next = JSON.parse(json); strings = next.strings || {}; state = next; updateControls(); refreshPopover?.(); };
...
document.addEventListener('keydown', event => {
  ...
  if (popover && !event.target.matches('input') && ['ArrowDown', 'ArrowUp'].includes(event.key)) {
    ...
    buttons[(index + (event.key === 'ArrowDown' ? 1 : -1) + buttons.length) % buttons.length]?.focus({ preventScroll: true }); event.preventDefault();
  }
});
