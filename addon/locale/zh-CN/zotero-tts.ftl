...
ztts-help-open-expanded =
    .value = ?
    .help = 每次打开悬浮面板时显示语音服务、语言和声音三行。点击选项或按 Shift+O 可在本次打开期间收起或展开。设置更改在下次打开播放器时生效。
ztts-one-voice =
    .label = 所有文档使用同一语音
ztts-help-one-voice =
    .value = ?
...
ztts-help-sentence-pause =
    .value = ?
    .help = 每个语音读完一句后等多久再读同一段的下一句，按 1 倍速计；读得越快，停顿按比例缩短。关闭时：句与句之间不停顿。
ztts-help-paragraph-pause =
    .value = ?
    .help = 段落开头的整段停顿，按 1 倍速计，在这里代替句间停顿，比句间停顿短也照此停顿；读得越快，按比例缩短。关闭时：段落开头不停顿。
ztts-skipped-lines =
...
ztts-help-key-stop =
    .value = ?
    .help = 一次关闭所有标签页里的播放器。每个标签页的阅读位置都会保留，再次开始朗读时从原处继续。没有播放器打开时，这个键保持原来的作用。
...
ztts-help-key-player-layout =
    .value = ?
    .help = 播放器打开时，按顶部栏 → 底部栏 → 悬浮面板循环切换。所有播放器共用此布局，重启后仍会保留。
