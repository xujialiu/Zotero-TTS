...
ztts-help-fish =
    .value = ?
    .help = 密钥在 fish.audio 账号的 Developers 页面上。列出的语音：你在那里做的、下面粘贴的、以及模型自带的默认声音，都逐词高亮。免费模型不花钱，但会保留你的请求文本，不保证速度，也可能随时结束；付费模型按文本字节计费，一个汉字算三个字节。
ztts-fish-free-only =
    .label = 只用免费模型
...
ztts-fish-voices-input =
    .placeholder = fish.audio 上的语音 id 或链接
ztts-help-fish-voices =
    .value = ?
    .help = 在 fish.audio 上打开一个语音，把它的链接或 id 粘到这里；多个用逗号或空格隔开。自己做的语音不用粘。
ztts-heading-fish-speech = 自建 Fish Speech 服务器
