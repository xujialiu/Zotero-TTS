...
description: "Brings the Chinese pages — README.zh.md, docs/PHILOSOPHY.zh.md and tutorials/<name>.zh.md — up to an English edit by the project's translation rules (the delta translated, every other byte kept, the hashes pinned with npm run docs:pin, test/docs-translation.test.ts green) and reports what changed. Use whenever README.md, docs/PHILOSOPHY.md or a tutorial changed and its Chinese page is stale, or a tutorial has no Chinese page yet. It edits only the .zh.md files, never an English page or the code. A session running Fable or Astra hands the translation over; any other model translates in place by these rules."
model: sonnet
