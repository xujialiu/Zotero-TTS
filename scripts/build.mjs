...
import { buildDateString, buildTimeString } from './build-date.mjs';
...
await esbuild.build({
  ...
  external: [],
  // The About section of the settings pane shows when this ran
  // (ui/about-rows.ts); nothing else can know it, the xpi carries no other
  // trace of when it was made
  define: { __BUILD_DATE__: JSON.stringify(buildDateString()), __BUILD_TIME__: JSON.stringify(buildTimeString()) },
  logLevel: 'warning',
});
